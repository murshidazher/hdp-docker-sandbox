...
list_installed_services() {
  ...
  for host in $(cat /tmp/list_all_hosts); do
    echo -e "\n$(tput setaf 2)$host$(tput sgr 0)\n\n"
    curl -s -H "X-Requested-By:ambari" -u $AMBARI_ADMIN_USER:$AMBARI_ADMIN_PASSWORD -i -X GET http://$AMBARI_HOST:8080/api/v1/clusters/$CLUSTER_NAME/hosts/$host/host_components | grep component_name | cut -d',' -f1 | tr ':' '|'
  done >/tmp/list_host_components
  if [ "$1" == "show" ]; then
...
restart_stale_configs() {
  ...
  for URL in $(cat /tmp/list_of_components_final); do
...
if [ $# -eq 1 -a "$1" == "startall" ]; then
  service_action "STARTED"
elif [ $# -eq 1 -a "$1" == "stopall" ]; then
  service_action "INSTALLED"
elif [ $# -eq 1 -a "$1" == "listall" ]; then
  list_installed_services show
elif [ $# -eq 2 -a "$1" == "start" ]; then
  ...
  HOST_EXIST_STAT=$?
  if [ $SERVICE_EXIST_STAT -eq 0 -a $HOST_EXIST_STAT -eq 0 ]; then
...
elif [ $# -eq 3 -a "$1" == "add" ]; then
  SERVICE_NAME=$(echo $2 | tr [a-z] [A-Z])
  grep -wq $3 /tmp/list_all_hosts
  HOST_EXIST_STAT=$?
  if [ $HOST_EXIST_STAT -eq 0 ]; then
...
elif [ $# -eq 2 -a "$1" == "restart" -a "$2" == "refresh" ]; then
  restart_stale_configs
elif [ $# -eq 4 -a "$1" == "backup" -a "$3" == "mysql" -o "$3" == "postgresql" ]; then
  backup_db $2 $3 $4
else
  usage
fi
